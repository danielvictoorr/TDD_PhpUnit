...
class OrderController extends Controller
{
    ...
    public function sendOrderAction(Request $request)
    {
        if (!$this->isValid($request)) {
            return new Response(Response::BAD_REQUEST, 'Invalid parameters'); //validaçoes de campos
        }

        $customerID = $request->get('customer_id');
        $customer = $this->customerRepository->findByID($customerID); //busca os dados do clientes
        ...
        $item = $this->itemRepository->findByID($itemID); //busca o item

        if (!$item) {
            return new Response(Response::NOT_FOUND, 'Item not found'); //valida se o item existe
        }
        ...
        try {
            $order = $this->orderService->process( //classe que processa o pedido
                $customer,
                $item,
                $description,
                $creditCard
